import { EditorSession, DefaultDOMElement, BodyScrollPane, DOMSelection, uuid } from 'substance'
...
/*
  ...
  TODO: Use more explicit API like this

  ```
  // Forms configuration part
  var forms = new Forms()
    .enableXrefs({comment: 'comment-icon'})

  // Forms execution part
  var editables = document.querySelectorAll('[editable]')
  editables.forEach(funciton(editable) {
    forms.makeEditable(editable)
  })
  ```
*/
class Forms extends BodyScrollPane {
  ...
  getHTML(editorId) {
    return this.editables[editorId].getHTML()
  }

  setHTML(editorId, html) {
    this.editables[editorId].setProps({
      html: html
    })
  }
  ...
  _onSelectionChanged() {
    ...
    this.emit('selection:changed', {
      activeAnnotation,
      fieldName: selectionState.selection.containerId
    })
  }
  ...
  _emitSelectionPositioned(hints) {
    hints.editorId = hints.surface.context.editorId
    this.emit('selection:positioned', hints)
  }

  createHTMLEditor(editorId, html, el) {
    el = DefaultDOMElement.wrapNativeElement(el)
    let htmlEditor = new EditComment(this, { html, editorId }).mount(el)
    this.editables[editorId] = htmlEditor
    return htmlEditor
  }
