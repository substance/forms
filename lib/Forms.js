...
export default class Forms extends EventEmitter {
  ...
  _onSelectionChanged(params) {
    ...
    // Blur the previous active editor
    if (this.activeEditor && this.activeEditor !== params.editorId) {
      console.log('hiding overlays for', this.activeEditor)
      this._editables[this.activeEditor].hideOverlays()
    }
    this.activeEditor = params.editorId
